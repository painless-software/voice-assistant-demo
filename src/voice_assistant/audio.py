...
Twilio Media Streams deliver audio as:
  ...
  • mono / 8-bit samples (after μ-law decoding → 16-bit PCM)
...
def twilio_mulaw_to_gemini_pcm(mulaw_b64: str) -> bytes:
    """
    Convert a base64-encoded μ-law/8kHz chunk (as sent by Twilio) to
    linear PCM 16-bit / 16 kHz bytes suitable for Gemini Live API.
    """
    mulaw_bytes = base64.b64decode(mulaw_b64)
    # μ-law → 16-bit linear PCM at 8 kHz
...
def gemini_pcm_to_twilio_mulaw_b64(pcm_24k: bytes) -> str:
    ...
    to a base64-encoded μ-law/8kHz string ready to be sent back to Twilio.
    """
    # Downsample 24 kHz → 8 kHz
    pcm_8k, _ = audioop.ratecv(pcm_24k, 2, 1, 24000, 8000, None)
    # 16-bit linear PCM → μ-law
